 ...
 *  Info:     Timer0 clk source froms the external IO
 *            Input CLK: IOA.27
 *            Output toggle: IOA.8
 *
 *
...
int main()
{
  ...
  /*
	 ...
	 */
  TIMER_Open(TM0);
  MODIFY_REG(IOFUNC->CTRL2, GPIOFUNC_CTRL2_EXT0_PINSEL1_MSK, GPIOFUNC_CTRL2_EXT0_IOA27);    // Ext. CLK input pin: IOA.27
  MODIFY_REG(TM0->CTRL, TIMER_CTRL_CLK_SEL_MSK, TIMER_CTRL_CLK_SEL_EXTCLK);
	TM0->PLOAD = 0x0;
	TM0->COUNT = 0x0;
