...
static const uint16_t SW_Mixer_5Ch_Table[] =
{
  0,     0,    0, 0,
  2048,  2048, 0, 0,
  4096,  4096, 0, 0,
...
void APP_SwMixer_5ch(int16_t *PcmInBuffer1, int16_t *PcmInBuffer2,
                     int16_t *PcmInBuffer3, int16_t *PcmInBuffer4,
                     int16_t *PcmInBuffer5, int16_t *DstBufAddr, uint16_t DataLen)
{
  uint32_t SignFlag;
  uint32_t BuffIdx;
  int32_t PcmData;
  ...
  {
    PcmData = 0;
    if (PcmInBuffer1 != 0) PcmData += PcmInBuffer1[BuffIdx];
    if (PcmInBuffer2 != 0) PcmData += PcmInBuffer2[BuffIdx];
    if (PcmInBuffer3 != 0) PcmData += PcmInBuffer3[BuffIdx];
    if (PcmInBuffer4 != 0) PcmData += PcmInBuffer4[BuffIdx];
    if (PcmInBuffer5 != 0) PcmData += PcmInBuffer5[BuffIdx];

    PcmData >>= 3;  // 5通道時，平均值應位移3 (2^3 = 8)

    SignFlag = (PcmData <= 0);
    if (SignFlag) PcmData = -PcmData;

    SwMixerTableIdx = PcmData >> 11;
    if (SwMixerTableIdx != 0)
    {
      SwMixerTableIdx <<= 2;
      Boundary = SW_Mixer_5Ch_Table[SwMixerTableIdx++];
      ReMappingBoundary = SW_Mixer_5Ch_Table[SwMixerTableIdx++];
      Divisor = SW_Mixer_5Ch_Table[SwMixerTableIdx];
      PcmData = ReMappingBoundary + ((PcmData - Boundary) >> Divisor);
    }
    ...
    DstBufAddr[BuffIdx] = (int16_t)PcmData;
  }
}
