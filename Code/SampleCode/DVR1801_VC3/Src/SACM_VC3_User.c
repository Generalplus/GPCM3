...
{
	mVc3DsAdcDmaHandle = DMA1;
	mVc3DacCh0DmaHandle = DMA1;
	mVc3DacCh1DmaHandle = DMA1;
	mVc3DacTimerHandle = TM1;
  ...
  // DAC init
	...
	// SET_BIT(CLOCK->AHBCKEN, CLOCK_AHBCKEN_SPU_CLK_ENABLE);             // SPU CLK enable for DAC out.
...
/*
  ...
  // SET_BIT(mVc3DsAdcDmaHandle->CTRL,DMA_CTRL_DMA_DUAL_ENABLE);      // DMA Dual Buffer Mode Enabled.
  // SET_BIT(mVc3DsAdcDmaHandle->CTRL,DMA_CTRL_DMA_CIRC_ENABLE);      // DMA circular mode enabled.
  // SET_BIT(mVc3DsAdcDmaHandle->CTRL,0x80);                          // DMA continuous mode enabled.
  ...
  DMA_InstallIsrService(mVc3DsAdcDmaHandle, SACM_VC3_GetAdc_DmaIsrService);
    ...
    // SET_BIT(mVc3DacCh0DmaHandle->CTRL,DMA_CTRL_DMA_DUAL_ENABLE);      // DMA Dual Buffer Mode Enabled.
    // SET_BIT(mVc3DacCh0DmaHandle->CTRL,DMA_CTRL_DMA_CIRC_ENABLE);      // DMA circular mode enabled.
    ...
    CLEAR_BIT(DAC->CTRL, DAC_CTRL_DAC_CH1_DMA_ENABLE);                // DAC_Ch1 DMA request disabled.
		DMA_Init(mVc3DacCh1DmaHandle, DMA_REQSEL_DAC_CH1, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
    // SET_BIT(DAC->CTRL,0x60000);                        // DAC Ch0 & Ch1 DMA continue mode enabled
    // SET_BIT(mVc3DacCh1DmaHandle->CTRL,DMA_CTRL_DMA_DUAL_ENABLE);      // DMA Dual Buffer Mode Enabled.
    // SET_BIT(mVc3DacCh1DmaHandle->CTRL,DMA_CTRL_DMA_CIRC_ENABLE);      // DMA circular mode enabled.
