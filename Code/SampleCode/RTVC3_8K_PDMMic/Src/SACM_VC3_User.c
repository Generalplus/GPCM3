...
void VC3_CB_Init(const SACM_VC3_API_WORKING_RAM *Vc3ApiWorkingRam)
{
  ...
  // PDM initial(IOA[23:22])
  MODIFY_REG(IOFUNC->CTRL0, GPIOFUNC_CTRL0_PDM_IOSEL_MSK, GPIOFUNC_CTRL0_PDM_IOA22_23);    //  IOA.5/22 = DATA/ IOA.6/23= CLK
 	DS_ADC->CTRL |= DS_ADC_CTRL_PDM_ENABLE | DS_ADC_CTRL_PDM_L_RISING_R_RISING | DS_ADC_CTRL_PDM_RCH_DATA_EQU_LCH;   //PDM Mic init.
...
void VC3_CB_StartPlay(const SACM_VC3_API_WORKING_RAM *Vc3ApiWorkingRam)
{
  ...
  DMA_InstallIsrService(mVc3DsAdcDmaHandle, SACM_VC3_GetAdc_DmaIsrService);
	...
	while(DAC_ReadyFlag < 30);    // Wait until ADC ready & stable to avoid "pop" noise by Kim
