...
void SACM_MIXER_Ch0_Initial(SACM_MIXER_CH0_WORKING_RAM *MixerWorkingRam, int8_t *MixerPcmBufPtr, int16_t FrameSize, int16_t Bits)
{
	...
	if(Bits == 32)
	{
		mMixerCh0WorkingRamPtr->TypeSize = FrameSize*4;											// int32_t: 4-bytes
	}
	else
	{
...
void SACM_MIXER_Ch0_Play(uint8_t DacChannelNo, uint8_t AutoRampUpDownCtrl)
{
   ...
   */
	for(iCount = 0; iCount < (mMixerCh0WorkingRamPtr->FrameSize); iCount++)
  {
...
/*
  if(((mMixerCh0Status & MIXER_CH0_PLAY_FLAG) == 0) || ((mMixerCh0Status & MIXER_CH0_ENCODE_WORK_FLAG) == 0) || ((mMixerCh0Status & MIXER_CH0_DECODE_WORK_FLAG) == 0) || ((mMixerCh0Status & MIXER_CH0_PAUSE_FLAG) != 0))
	{
    return;
	}
*/
...
void SACM_MIXER_Ch0_GetAdc_DmaIsrService()
{
	...
	if((mMixerCh0Status & MIXER_CH0_PLAY_FLAG)	!= 0)
  {
	  if((mMixerCh0Status & MIXER_CH0_ADCIN_BUF_EVEN_FLAG) == 0)
		...
		mMixerCh0Status |= MIXER_CH0_ENCODE_WORK_FLAG;
