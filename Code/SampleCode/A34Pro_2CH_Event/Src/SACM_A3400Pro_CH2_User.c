...
void static RampUpDacCh0()
{
  ...
  DacDataTemp1 = DAC->DAC_CH0_DMA_DATA0 & 0xFFC0;
	...
	do
	{
	  ...
	  TIMER_Close(mA3400ProCh2DacTimerHandle);
	  DMA_Init(mA3400ProCh2DacCh0DmaHandle, DMA_REQSEL_DAC_CH0, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
	  SET_BIT(mA3400ProCh2DacCh0DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
	  DMA_Trigger(mA3400ProCh2DacCh0DmaHandle, (uint32_t)&DacDataBuf[0], (uint32_t)&DAC->DAC_CH0_DMA_DATA0, DataLen);
	  TIMER_Open(mA3400ProCh2DacTimerHandle);
		while(DMA_CheckBusy(mA3400ProCh2DacCh0DmaHandle) != 0);           // Wait DMAx busy
...
void static RampUpDacCh1()
{
  ...
  DacDataTemp1 = DAC->DAC_CH1_DMA_DATA0 & 0xFFC0;
	...
	do
	{
	  ...
	  TIMER_Close(mA3400ProCh2DacTimerHandle);
		DMA_Init(mA3400ProCh2DacCh1DmaHandle, DMA_REQSEL_DAC_CH1, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
		SET_BIT(mA3400ProCh2DacCh1DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
		DMA_Trigger(mA3400ProCh2DacCh1DmaHandle, (uint32_t)&DacDataBuf[0], (uint32_t)&DAC->DAC_CH1_DMA_DATA0, DataLen);
		TIMER_Open(mA3400ProCh2DacTimerHandle);
		while(DMA_CheckBusy(mA3400ProCh2DacCh1DmaHandle) != 0);           // Wait DMAx busy
	} while(DacDataTemp2 != 0);
}
...
void A3400Pro_CH2_CB_Init(const SACM_A3400PRO_WORKING_RAM *A3400ProWorkingRam)
{
  ...
  TIMER_Open(mA3400ProCh2DacTimerHandle);
	TIMER_SetFreq(mA3400ProCh2DacTimerHandle, (12000 * 4));	                 // Sample rate: 12000 x 4 = 48000(Hz), for H/W 4x upsampling
...
void A3400Pro_CH2_CB_StartPlay(const SACM_A3400PRO_WORKING_RAM *A3400ProCh2WorkingRam)
{
	...
	DacAutoRampUp(SACM_A3400Pro_CH2_GetStatus());                             // DAC auto ramp up

	if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH0_FLAG) != 0)
	{
	  CLEAR_BIT(DAC->CTRL, DAC_CTRL_DAC_CH0_DMA_ENABLE);                // DAC_Ch0 DMA request disabled.
	  TIMER_Close(mA3400ProCh2DacTimerHandle);
    while(DMA_CheckBusy(mA3400ProCh2DacCh0DmaHandle) != 0);
	  DMA_Init(mA3400ProCh2DacCh0DmaHandle, DMA_REQSEL_DAC_CH0, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
	  SET_BIT(mA3400ProCh2DacCh0DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
	  DMA_InstallIsrService(mA3400ProCh2DacCh0DmaHandle, SACM_A3400Pro_CH2_DmaIsrService);
	  DMA_EnableInt(mA3400ProCh2DacCh0DmaHandle);
	  NVIC_EnableIRQ(DMA3_IRQn);
	  DMA_Trigger(mA3400ProCh2DacCh0DmaHandle, (uint32_t)A3400ProCh2WorkingRam->PcmBufPtr, (uint32_t)&DAC->DAC_CH0_DMA_DATA0, A3400ProCh2WorkingRam->FrameSize);
    TIMER_Open(mA3400ProCh2DacTimerHandle);
	}

	if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH1_FLAG) != 0)
	{
	  CLEAR_BIT(DAC->CTRL, DAC_CTRL_DAC_CH1_DMA_ENABLE);                // DAC_Ch1 DMA request disabled.
	  TIMER_Close(mA3400ProCh2DacTimerHandle);
    while(DMA_CheckBusy(mA3400ProCh2DacCh1DmaHandle) != 0);
		DMA_Init(mA3400ProCh2DacCh1DmaHandle, DMA_REQSEL_DAC_CH1, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
		SET_BIT(mA3400ProCh2DacCh1DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
    if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH0_FLAG) == 0)
		{
	    DMA_InstallIsrService(mA3400ProCh2DacCh1DmaHandle, SACM_A3400Pro_CH2_DmaIsrService);
	    DMA_EnableInt(mA3400ProCh2DacCh1DmaHandle);
	    NVIC_EnableIRQ(DMA4_IRQn);
		}
		DMA_Trigger(mA3400ProCh2DacCh1DmaHandle, (uint32_t)A3400ProCh2WorkingRam->PcmBufPtr, (uint32_t)&DAC->DAC_CH1_DMA_DATA0, A3400ProCh2WorkingRam->FrameSize);
    TIMER_Open(mA3400ProCh2DacTimerHandle);
	}
...
{
	if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_PLAY_FLAG) != 0)
  {
	  if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH0_FLAG) != 0)
	  {
		  DMA_Close(mA3400ProCh2DacCh0DmaHandle);
		}

	  if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH1_FLAG) != 0)
	  {
		  DMA_Close(mA3400ProCh2DacCh1DmaHandle);
		}

    DacAutoRampDown(SACM_A3400Pro_CH2_GetStatus());	                       // DAC auto ramp down
	  if((SACM_A3400Pro_GetStatus() & A3400PRO_PLAY_FLAG) == 0)
	  {
      #if AUDIO_PWM
        DAC_AudioPwm_IP_Disable();
...
void A3400Pro_CH2_CB_SendDac_DmaIsr(const SACM_A3400PRO_WORKING_RAM *A3400ProCh2WorkingRam, int16_t *SrcBufAddr, uint16_t DataLen)
{
	if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH0_FLAG) != 0)
	{
		while(DMA_CheckBusy(mA3400ProCh2DacCh0DmaHandle) != 0);                // Check DMAx busy
		DMA_Trigger(mA3400ProCh2DacCh0DmaHandle, (uint32_t)SrcBufAddr, (uint32_t)&DAC->DAC_CH0_DMA_DATA0, DataLen);
	}

	if((SACM_A3400Pro_CH2_GetStatus() & A3400PRO_ENABLE_DAC_CH1_FLAG) != 0)
	{
		while(DMA_CheckBusy(mA3400ProCh2DacCh1DmaHandle) != 0);                // Check DMAx busy
    DMA_Trigger(mA3400ProCh2DacCh1DmaHandle, (uint32_t)SrcBufAddr, (uint32_t)&DAC->DAC_CH1_DMA_DATA0, DataLen);
	}
}
