...
#if defined(__CC_ARM)
__align(4) GP_EVENTOR_WORKING_RAM GpEventorCh0WorkingRam;
__align(4) GP_EVENTOR_CH0_SWPWM_WORKING_RAM GpEventorCh0SwPwmWorkingRam;
 ...
 *   None.
 */
void A1801_User_A1801WithGpEventServerLoop()
{
...
void A1801_CB_Init(const SACM_A1801_WORKING_RAM *A1801WorkingRam)
{
  ...
  TIMER_Open(mA1801DacTimerHandle);
	TIMER_SetFreq(mA1801DacTimerHandle, (44100 * 4));	                 // Sample rate: 16000 x 4 = 64000(Hz), for H/W 4x upsampling
...
void A1801_CB_StartPlay(const SACM_A1801_WORKING_RAM *A1801WorkingRam)
{
  ...
  DacAutoRampUp(SACM_A1801_GetStatus());                             // DAC auto ramp up
	...
	// GP_EVENTOR_Start(EVENTOR_CH0, (uint8_t *)(&A1801WorkingRam->SpeechDataAddr[A1801WorkingRam->DataLen]));
}
...
void A1801_CB_DecodeProcess(const SACM_A1801_WORKING_RAM *A1801WorkingRam, int16_t *DstBufAddr, int16_t *SrcBufAddr)
{
  ...
  GpEventFlag = SACM_A1801_DecodeProcess(DstBufAddr, SrcBufAddr);
