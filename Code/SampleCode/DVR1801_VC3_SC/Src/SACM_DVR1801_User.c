 ...
 *---------------------------------------------------------------------------------------*/
SACM_DVR1801_WORKING_RAM *mDvr1801WorkingRamPointer;
...
void VC3_ChangeSpeed_GetDataFunc(int16_t *DstBufAddr, uint16_t DataLen)
{
	...
	FrameLen = mDvr1801WorkingRamPointer->HalfWordsPerFrame;

	for(iCount = 0; iCount < FrameLen; iCount++)
  {
		DstBufAddr[iCount] = mDvr1801WorkingRamPointer->SpeechDataAddr[mDvr1801WorkingRamPointer->DecodeCount + iCount];
	}

	mDvr1801WorkingRamPointer->DecodeCount += FrameLen;
	SACM_DVR1801_DecodeProcess(DstBufAddr, DstBufAddr);
...
void static RampUpDacCh0()
{
  ...
  DacDataTemp1 = DAC->DAC_CH0_DMA_DATA0 & 0xFFC0;
	...
	do
	{
	  ...
	  SET_BIT(mDvr1801DacCh0DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
  ...
  DacDataTemp1 = DAC->DAC_CH1_DMA_DATA0 & 0xFFC0;
	...
	do
	{
	  ...
	  CLEAR_BIT(DAC->CTRL, DAC_CTRL_DAC_CH1_DMA_ENABLE);                 // DAC_Ch1 DMA request disabled.
		...
		SET_BIT(mDvr1801DacCh1DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
...
void DVR1801_CB_StartRecord(const SACM_DVR1801_WORKING_RAM *Dvr1801WorkingRam)
{
	...
	SET_BIT(mDvr1801DsAdcDmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
...
void DVR1801_CB_GetData(const SACM_DVR1801_WORKING_RAM *Dvr1801WorkingRam, int16_t *DstBufAddr, int16_t *SrcDataAddr, uint16_t DataLen)
{
  ...
  //*/
	...
	/*
  DMA_Init(DMA4, DMA_REQSEL_MEM, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_INC);
	DMA_Trigger(DMA4, (uint32_t)SrcDataAddr, (uint32_t)DstBufAddr, DataLen);
	while(DMA_CheckBusy(DMA4) != 0);
	DMA_Close(DMA4);
	*/
...
void DVR1801_CB_DecodeProcess(const SACM_DVR1801_WORKING_RAM *Dvr1801WorkingRam, int16_t *DstBufAddr, int16_t *SrcBufAddr)
{
	...
	{
		mDvr1801WorkingRamPointer = (SACM_DVR1801_WORKING_RAM*)Dvr1801WorkingRam;
		mDvr1801WorkingRamPointer->DecodeCount -= mDvr1801WorkingRamPointer->HalfWordsPerFrame;

		SACM_VC3_ChangeSpeedProcess(DstBufAddr, DstBufAddr, VC3_ChangeSpeed_GetDataFunc);

		// SACM_VC3_RobotEffect1(DstBufAddr, DstBufAddr, VC3_FRAME_SIZE);
		// SACM_VC3_EchoProcess(DstBufAddr, DstBufAddr);

		// SACM_VC3_ShiftPitchProcess(DstBufAddr, DstBufAddr);
		APP_SwVolCtrl_VolProcess(1, DstBufAddr, VC3_FRAME_SIZE);
	}
	else
	{
