...
int main()
{
   ...
   * IOA[18:9] output low
	 */
  GPIO_SetMode(GPIOA, 0x00010000, GPIO_MODE_OUTPUT);     //IOA.16 = Output for test
  MODIFY_REG(GPIOA->OBUF, 0x00010000, 0);


	KeyScan_Initial(&KeyScanWorkingRam);                     // key scan init.
  //GPIO_SetMode(GPIOA, BIT16, GPIO_MODE_OUTPUT);
  ...
  while(1)
	{
        ...
        //        SD_PlayMP3(PlayIndex);
			...
			case 0x04:                                           // IOA26+VDD: Play next song
				PlaySD = 0;
				PlayCon = 0;
				MP3_Stop();
