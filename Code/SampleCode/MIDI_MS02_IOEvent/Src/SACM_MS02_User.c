...
// For IO Event
#if defined(__CC_ARM)
__align(4) MS02_EVENTOR_WORKING_RAM GpEventorCh2WorkingRam;
__align(4) MS02_EVENTOR_CH2_SWPWM_WORKING_RAM MS02EventCh2WorkingRam;
#elif defined(__GNUC__)
__attribute__ ((aligned (4))) MS02_EVENTOR_WORKING_RAM GpEventorCh2WorkingRam;
__attribute__ ((aligned (4))) MS02_EVENTOR_CH2_SWPWM_WORKING_RAM MS02EventCh2WorkingRam;
#endif
...
void MS02_CB_Init(const SACM_MS02_WORKING_RAM *MS02WorkingRam, uint8_t AudOutType, uint8_t DacChannelNo, uint32_t Timer_SR)
{
  ...
  MS02_EVENTOR_Initial(MS02_EVENT_CH2, &GpEventorCh2WorkingRam, (int16_t*)&MS02EventCh2WorkingRam, MS02_EVENT_IO_NUM, MS02_EVENT_SW_PWM_LEVEL);
...
void MS02_CB_SongEvent(short R_CallBack_Event, short R_CMD_Code)
{
  ...
  mSongEvent = ((R_CMD_Code & 0x00FF)<< 8) + ((R_CMD_Code & 0xFF00)>> 8); // Change R_CMD_Code to bit[15:8] = sub-Index, bit[7:0] = Main-Index
