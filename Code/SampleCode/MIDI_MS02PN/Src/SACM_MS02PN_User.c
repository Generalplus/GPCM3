...
void MS02PN_CB_Init(const SACM_MS02PN_WORKING_RAM *MS02PNWorkingRam, uint8_t AudOutType, uint8_t DacChannelNo, uint32_t Timer_SR)
{
	...
	mMS02PNDacTimerHandle = TM0;

  TIMER_Open(mMS02PNDacTimerHandle);
	TIMER_SetFreq(mMS02PNDacTimerHandle, Timer_SR);
  ...
  if(DacChannelNo == MS02_CH0)
	...
	{
    PN_AUD_OUT_TYPE =  AUD_OUT_PWM;
	}
	else
	{
    PN_AUD_OUT_TYPE =  AUD_OUT_DAC;
	}
/*
	if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) != 0)
	{
	  DMA_Init(mMS02PNDacCh0DmaHandle, DMA_REQSEL_DAC_CH0, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
	  SET_BIT(mMS02PNDacCh0DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
	  DMA_InstallIsrService(mMS02PNDacCh0DmaHandle, SACM_MS02PN_DmaIsrService);
	  DMA_EnableInt(mMS02PNDacCh0DmaHandle);
	  NVIC_EnableIRQ(DMA3_IRQn);
	}

	if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH1_FLAG) != 0)
	{
		DMA_Init(mMS02PNDacCh1DmaHandle, DMA_REQSEL_DAC_CH1, DMA_SRC_DATA_16B, DMA_SRC_ADDR_INC, DMA_DST_DATA_16B, DMA_DST_ADDR_FIX);
		SET_BIT(mMS02PNDacCh1DmaHandle->CTRL,DMA_CTRL_DMA_CONTINUE_ENABLE);    // DMA continuous mode enabled.
    if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) == 0)
		{
	    DMA_InstallIsrService(mMS02PNDacCh1DmaHandle, SACM_MS02PN_DmaIsrService);
	    DMA_EnableInt(mMS02PNDacCh1DmaHandle);
...
{
	if(PN_AUD_OUT_TYPE == AUD_OUT_PWM)                                   // For AUD_OUT_PWM, Kim 2020.12.02
	{
  ...
  DacAutoRampUp(SACM_MS02PN_GetStatus());                             // DAC auto ramp up

  if((SACM_MS02PN_GetStatus() & MS02_PN_STOP_FLAG) != 0)
  {
    SACM_MS02PN_ClearStatus(MS02_PN_STOP_FLAG);
    ...
    if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) != 0)
    {
      ...
      DMA_EnableInt(mMS02PNDacCh0DmaHandle);
      NVIC_EnableIRQ(DMA3_IRQn);
      DMA_Trigger(mMS02PNDacCh0DmaHandle, (uint32_t)MS02PNWorkingRam->DecodeOutBuf2Ptr, (uint32_t)&DAC->DAC_CH0_DMA_DATA0, MS02_OUT_BUFFER_SIZE);
    }
    ...
    if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH1_FLAG) != 0)
    {
      ...
      DMA_Trigger(mMS02PNDacCh1DmaHandle, (uint32_t)MS02PNWorkingRam->DecodeOutBuf2Ptr, (uint32_t)&DAC->DAC_CH1_DMA_DATA0, MS02_OUT_BUFFER_SIZE);
    }
  }
}
...
{
	if((SACM_MS02PN_GetStatus() & MS02_PLAY_FLAG) != 0)
  {
	  if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) != 0)
	  {
		  DMA_Close(mMS02PNDacCh0DmaHandle);
		}

	  if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH1_FLAG) != 0)
	  {
		  DMA_Close(mMS02PNDacCh1DmaHandle);
		}

    DacAutoRampDown(SACM_MS02PN_GetStatus());	                       // DAC auto ramp down
    if(PN_AUD_OUT_TYPE == AUD_OUT_PWM)                                // For AUD_OUT_PWM, Kim 2020.12.02
    {
      DAC_AudioPwm_IP_Disable();
    }
...
void MS02PN_CB_Resume(const SACM_MS02PN_WORKING_RAM *MS02PNWorkingRam)
{
	if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) != 0)
	{
    DMA_Trigger(mMS02PNDacCh0DmaHandle, (uint32_t)MS02PNWorkingRam->DecodeOutBuf1Ptr, (uint32_t)&DAC->DAC_CH0_DMA_DATA0, MS02_OUT_BUFFER_SIZE);
	}

	if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH1_FLAG) != 0)
	{
		DMA_Trigger(mMS02PNDacCh1DmaHandle, (uint32_t)MS02PNWorkingRam->DecodeOutBuf1Ptr, (uint32_t)&DAC->DAC_CH1_DMA_DATA0, MS02_OUT_BUFFER_SIZE);
	}
}
...
void MS02PN_CB_SendDac_DmaIsr(const SACM_MS02PN_WORKING_RAM *MS02PNWorkingRam, int16_t *SrcBufAddr, uint16_t DataLen)
{
	if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) != 0)
	{
		while(DMA_CheckBusy(mMS02PNDacCh0DmaHandle) != 0);                // Check DMAx busy
		DMA_Trigger(mMS02PNDacCh0DmaHandle, (uint32_t)SrcBufAddr, (uint32_t)&DAC->DAC_CH0_DMA_DATA0, DataLen);
	}

	if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH1_FLAG) != 0)
	{
		while(DMA_CheckBusy(mMS02PNDacCh1DmaHandle) != 0);                // Check DMAx busy
		DMA_Trigger(mMS02PNDacCh1DmaHandle, (uint32_t)SrcBufAddr, (uint32_t)&DAC->DAC_CH1_DMA_DATA0, DataLen);
	}
}
...
void MS02PN_CB_DecodeProcess(int16_t* DstBufAddr, int16_t OutBufSize)
{
  SACM_MS02PN_DecodeProcess(DstBufAddr, OutBufSize);
 	APP_SwVolCtrl_VolProcess(1, DstBufAddr, MS02_OUT_BUFFER_SIZE);
}
