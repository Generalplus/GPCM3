 ...
 *   March 22, 2024
 * @Abstract:
 *    2024.03.22 Modify for SD card support by Kim.
 *
 **************************************************************************************************/
...
extern uint16_t SDbuf[];
extern uint16_t SDDataInx;
extern uint16_t PlaySD;
extern void ReadA18SDLoop(void);
extern void WriteA18SDLoop(void);
extern void WriteA18SDEnd(uint8_t *);
...
void DVR1801_CB_StopRecord(const SACM_DVR1801_WORKING_RAM *Dvr1801WorkingRam, uint32_t RecDataLen)
{
	...
	WriteA18SDEnd((uint8_t*)&RecDataLen);
}
  ...
  ///*
	uint32_t iCount;
		...
		if(PlaySD)
		{
			for(iCount = 0; iCount < DataLen; iCount++)
			{
				DstBufAddr[iCount] = SDbuf[SDDataInx++];
				if(SDDataInx>=256) ReadA18SDLoop();
			}
		}
		else
		{
...
void DVR1801_CB_WriteData(const SACM_DVR1801_WORKING_RAM *Dvr1801WorkingRam, int16_t *DstDataAddr, int16_t *SrcBufAddr, uint16_t DataLen)
{
  ...
  if(PlaySD)
  {
      for(iCount = 0; iCount < DataLen; iCount++)
      {
        SDbuf[SDDataInx++] = SrcBufAddr[iCount];
        if(SDDataInx>=256) WriteA18SDLoop();
      }
  }
  else
  {
