...
// 	SPI1_SetSpeed(SPI_SPEED_256);//设置到低速模式
	SPI_Flash_SetClkDiv(SPI_CLKSEL_PCLK_DIV256);
}
//SD卡正常工作的时候,可以高速了
void SD_SPI_SpeedHigh(void)
{
	SPI_Flash_SetClkDiv(SPI_CLKSEL_PCLK_DIV4);//设置到高速模式
...
void SD_SPI_Init(void)
{
	...
	SPI_Flash_Open();
	GPIO_SetMode(GPIOA,BIT22,GPIO_MODE_OUTPUT);		//SCL,SDA Oput
	MODIFY_REG(GPIOA->OBUF, BIT22, BIT22);
